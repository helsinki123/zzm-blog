...
在各种放射检查技术中择优选择，比如胸部检查，有透视、普通X射线摄影、CR（计算机X线摄影）、DR（数字X线摄影）甚至CT等多种选择，这时要充分考虑经济可承受性、对某种疾病的特异性和敏感性、检查导致的受检者剂量的高低、需要复查的可能性、需要进行的各种检查的总体潜在危害等等。健康体检中对放射检查的正当性判断是一个综合的过程，辐射仅仅是其中必须考虑的因素之一。
有效剂量：为了描述辐射所致机体健康危害的大小，定量地评价辐射照射有可能导致的风险的大小，在辐射防护评价中，人为地引入了有效剂量的概念
指标：普通公众每年受到天然本底辐射的有效剂量为2.4mSv （世界平均值）。

放射诊疗：监管机构端，医疗机构端，协作机构（检测技术有限公司，专门检测放射设备的安全性，个人剂量检测报告），AI行为管理（将异常数据以列表的形式综合展示）
```
# 4/20星期三
## 项目启动与vue.config.js
